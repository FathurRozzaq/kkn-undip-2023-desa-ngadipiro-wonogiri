(function (w, d) {
    ...
    ! function (fb, fc, fd, fe, ff, fg) {
        ...
        function fi(fl, fm, fn, fo) {
            return fm && fc.getElementById(fm) || (fo = fc.createElement(fl || "SCRIPT"), fm && (fo.id = fm), fn && (fo.onload = fn), fc.head.appendChild(fo)), fo || {}
        }
        fg = /p/.test(fc.readyState), fb.addEventListener("on" + fd in fb ? fd : "load", (function () {
            for (fg = 1; ff[0];) fh(ff.shift(), ff.shift())
        })), fh._ = fi, fb.defer = fh, fb.deferscript = function (fp, fq, fr, fs) {
            fh((function () {
                fi("", fq, fs).src = fp
            }), fr)
        }
    ...
    zaraz.track = async function (ee, ef, eg) {
        return new Promise(((eh, ei) => {
            ...
            for (const ek of [localStorage, sessionStorage]) Object.keys(ek || {}).filter((em => em.startsWith("_zaraz_"))).forEach((el => {
                try {
            ...
            Object.keys(zaraz.pageVariables).forEach((en => ej.data[en] = JSON.parse(zaraz.pageVariables[en])));
            Object.keys(zaraz.__zcl).forEach((eo => ej.data[`__zcl_${eo}`] = zaraz.__zcl[eo]));
    ...
    zaraz.set = function (er, es, et) {
        ...
        if (void 0 !== es) {
            et && "session" == et.scope ? sessionStorage && sessionStorage.setItem(prefixedKey, es) : et && "page" == et.scope ? zaraz.pageVariables[er] = es : localStorage && localStorage.setItem(prefixedKey, es);
            zaraz.__watchVar = {
                key: er,
                value: es
            }
    ...
    for (const {
            ...
            m: ex
        }) => ["debug", "set"].includes(ex)))) zaraz[ev](...ew);
    for (const {
            m: ey,
            a: ez
        } of zaraz.q) zaraz[ey](...ez);
    delete zaraz.q;
    delete zarazData.q;
    ...
    zaraz._processDataLayer = fK => {
        for (const fL of Object.entries(fK)) zaraz.set(fL[0], fL[1], {
            scope: "page"
        });
        if (fK.event) {
            if (zarazData.dataLayerIgnore && zarazData.dataLayerIgnore.includes(fK.event)) return;
            let fM = {};
            for (let fN of dataLayer.slice(0, dataLayer.indexOf(fK) + 1)) fM = {
                ...fM,
                ...fN
            };
            delete fM.event;
    ...
    zaraz._rl = function () {
        w.zaraz.listeners && w.zaraz.listeners.forEach((dJ => dJ.item.removeEventListener(dJ.type, dJ.callback)));
    ...
    zaraz._p({
        ...
        "f": [
            ["https://stats.g.doubleclick.net/g/collect?t=dc&aip=1&_r=3&v=1&_v=j86&tid=G-SEKJ4E9T4H&cid=cf4d1c46-1c82-41f8-8cd5-ba0d08585e46&_u=KGDAAEADQAAAAC%7E&z=1780298941", {}]
        ]
    })
})(window, document);
